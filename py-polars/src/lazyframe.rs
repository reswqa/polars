...
impl PyLazyFrame {
    ...
    fn shift(&self, n: PyExpr) -> Self {
        let ldf = self.ldf.clone();
        ldf.shift(n.inner).into()
    }

    fn shift_and_fill(&self, n: PyExpr, fill_value: PyExpr) -> Self {
        let ldf = self.ldf.clone();
        ldf.shift_and_fill(n.inner, fill_value.inner).into()
    }
